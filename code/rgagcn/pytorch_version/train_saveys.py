...
def evaluate_full_batch(model, minibatch, mode='val'):
    ...
    # print('node_target1',type(node_target1))
    # np.savetxt("C:/Users/Administrator/Desktop/图采样有向图代码/test data/分类散点图数据/GCN/label[702].txt", node_target1 ,fmt='%s', newline='\n')
    # np.savetxt("C:/Users/Administrator/Desktop/图采样有向图代码/test data/分类散点图数据/MDA-GCNGS/label[334].txt", node_target1 ,fmt='%s', newline='\n')
    # print('node_target]',node_target.shape)
    ...
    for n in node_target:
        ...
        ys, performances = metrics(to_numpy(labels[n, 1]), to_numpy(preds[n, 1]), model.sigmoid_loss)  #### metric.py代码
        # print(ys)
        # ys, performances = metrics(to_numpy(labels[n,0]), to_numpy(preds[n,0]), model.sigmoid_loss)
        # ys, performances = metrics(to_numpy(labels), to_numpy(preds), model.sigmoid_loss)
        # print('ys',ys[1])
        # print('ys',ys[1])
        # print('ys',ys[1].shape)
        # print('labels[n]',labels[n])
        print('labels[n]', labels[n].shape)
        
        
        ys_test = to_numpy(preds[n])
        #print('preds',ys_test)
        #print('preds',preds[n].shape)
        #ys = ys.detach().numpy()
        #preds[n] = np.array(preds[n].detach())
        #print('preds',type(preds[n]))
        #np.savetxt("C:/Users/Administrator/Desktop/图采样有向图代码/test data/分类散点图数据/MDA-GCNS/ys_test[2].txt", ys[1],fmt='%s', newline='\n')
        #np.savetxt("C:/Users/Administrator/Desktop/图采样有向图代码/test data/分类散点图数据/MDA-GCNS/ys_test[3].txt", ys_test,fmt='%s', newline='\n')
...
if __name__ == '__main__':
    ...
    print(ys_test[2])
    np.savez('ys.npz',ys_train, ys_test)
    # np.savetxt("E:/MDA-GCNFTG-main/GDA/data/用户使用测试/ys_train.txt", ys_train,fmt='%s', newline='\n')
    # ys = pd.DataFrame(ys_test).T.to_csv('分类图/ys_test.csv')
    # print(ys)
    # np.savetxt(r"分类图/ys_test.txt", ys_test, fmt='%s', newline='\n')
    # print('ys',ys_train)
    # print('ys', ys_test)
    ...
    # data1 = pd.DataFrame(zip(precision, recall), columns=['precision','recall']).to_csv(r'D:\小麦\MDA-GCNFTG-main\MDA-GCNFTG-main\评价指标统计图\prc\GPA_GCN1.csv', index=False)
    # fpr, tpr, thresholds = roc_curve(ys_test[0], ys_test[2])
    # data = pd.DataFrame(zip(ys_test[0], ys_test[2]), columns=['fpr','tpr']).to_csv(r'D:\小麦\MDA-GCNFTG-main\MDA-GCNFTG-main\评价指标统计图\roc\GPA_GCN3.csv', index=False)


    # np.savetxt("E:/MDA-GCNFTG-main/prediction_results/ys_test[1]g.txt", ys_test[1], fmt='%s', newline='\n')
